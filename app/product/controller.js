...
async function index(req, res, next) {
    try {
        ...
        if (q.length) {
            criteria = {
                ...criteria, 
                name: {$regex: `${q}`, $options: 'i'} //option i berarti tidak case sensitif (tidak memperhatikan huruf kapital)
            }
        }
...
async function store(req, res, next) {
    try {
        ...
        if (req.file) {
            ...
            // (2) pindahkan file ke lokasi parmanen
            const dest = fs.createWriteStream(target_path)
            // (3) pindahkan file dari src ke dest
            src.pipe(dest)
        ...
        if (req.file) {
            ...
            src.on('end', async () => {
                try {
                    ...
                    product = await Product.findOneAndUpdate({_id: req.params.id}, {...payload, image_url: filename},
